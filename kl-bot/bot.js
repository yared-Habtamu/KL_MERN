...
function coreButtonsFor(userId) {
  const deepLink = `${FRONTEND_BASE}/?userId=${encodeURIComponent(
    userId
  )}&action=handle_start`;
  return [
...
bot.onText(/\/deposit(?:\s+(.*))?/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "Open deposit page:",
    webAppButton(`${FRONTEND_BASE}/deposit`, "Deposit")
  );
});
...
bot.onText(/\/balance(?:\s+(.*))?/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "View your wallet:",
    webAppButton(`${FRONTEND_BASE}/wallet`, "Wallet")
  );
});
...
bot.onText(/\/todays_lotteries(?:\s+(.*))?/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "Today's lotteries:",
    webAppButton(`${FRONTEND_BASE}/`, "Lotteries")
  );
});
...
bot.onText(/\/withdraw(?:\s+(.*))?/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "Open withdraw page:",
    webAppButton(`${FRONTEND_BASE}/withdraw`, "Withdraw")
  );
});
...
bot.onText(/\/unsubscribe(?:\s+(.*))?/, (msg) => {
  ...
  const userId = msg.from && msg.from.id ? String(msg.from.id) : null;
  if (!userId)
    return bot.sendMessage(chatId, "Could not determine your user id.");
...
function sendReminders() {
  ...
  console.log(
    `Triggered reminders; messages attempted to ${count} users. ${new Date().toISOString()}`
  );
}

schedule.scheduleJob({ cron: DAILY_CRON, tz: TZ }, sendReminders);

console.log(`Reminder schedule: CRON=${DAILY_CRON}, TZ=${TZ}`);
...
console.log(
  `Telegram bot started (mode=${USE_WEBHOOK ? "webhook" : "polling"})`
);
