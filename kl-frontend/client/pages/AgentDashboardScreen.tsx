...
//           lotteriesApi.list(),
//           // get current profile; if it fails, continue with null
//           usersApi.getProfile().catch(() => null),
//         ]);
//         if (!mounted) return;
//         const uid =
//           (me && (me.id || me._id)) ||
//           (me && me.user && (me.user.id || me.user._id));
//         setAgentLotteries(
//           Array.isArray(all) && uid
//             ? all.filter((l: any) => String(l.agentId) === String(uid))
//             : [],
//         );
//       } catch (e) {
...
const AgentDashboardScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAgentLotteries(
          Array.isArray(all)
            ? all.filter((l: any) => l.agentId === myId || l.createdBy === myId)
            : [],
        );
      } catch (e) {
        console.error("Failed to load agent lotteries", e);
  ...
  const totalRevenue = agentLotteries.reduce(
    (sum, lottery) => sum + lottery.ticketPrice * lottery.soldTickets,
    0,
  );
  ...
  const totalTicketsSold = agentLotteries.reduce(
    (sum, lottery) => sum + (lottery.soldTickets || 0),
    0,
  );

  const completedLotteries = agentLotteries.filter(
    (l) => l.status === "ended" || l.status === "completed",
  ).length;
                ...
                <div key={lottery.id} className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm bg-kiya-dark rounded-lg p-3">
                    <div className="text-center">
                      ...
                      <p className="font-semibold text-kiya-teal">
                        {formatCurrency(
                          lottery.ticketPrice * lottery.soldTickets,
                        )}
                      </p>
                    </div>
                    <div className="text-center">
